...
Function gf_mgem
  ...
  // Defining Points
  // Superior Chamber
  x0 = X + -DISTANCE_HOLES / 4; y0 = Y;
  z0 = Z + DISTANCE_ELE / 2 + THICKNESS_PLA;
  ...
  // Inferior Chamber
  x0 = X -DISTANCE_HOLES / 4; y0 = Y;
  z0 = Z -DISTANCE_PAD / 2 - THICKNESS_PLA - THICKNESS_DIE;
  ...
  If (ID == 0)
    Physical Surface(219 + 8 * ID) = {100}; // Electrode
    Printf("Part of the first detector layer");
  EndIf
  Physical Surface(220 + 8 * ID) = {110}; // Upper Plate
  Physical Surface(221 + 8 * ID) = {196}; // Lower Plate
  If (ID == NTOT - 1)
    Physical Surface(222 + 8 * ID) = {206}; // Pad
